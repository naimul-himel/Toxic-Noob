...
def api_1(number):
    print(f"Sending SMS to {number} via API 1...")
    return True

def api_2(number):
    print(f"Sending SMS to {number} via API 2...")
    return True

def api_3(number):
    print(f"Sending SMS to {number} via API 3...")
    return True
...
def main():
    ...
    # Define number of APIs to use
    items = 3  # You can increase this number if you have more APIs
    finished = False
    
    # Running through all APIs
    allFuncs = globals()
    if check(sent):
        sys.exit()
    ...
    while True:
        for i in range(1, items+1):
            success = allFuncs[f"api_{i}"](number)
            if success:
                sent += 1
                if check(sent):
                    finished = True
